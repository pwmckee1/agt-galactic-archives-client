...
import { galacticCoordinatesRegex } from '@shared/helpers/game-helpers';
...
export function getGalacticCoordinateValidator(): ValidatorFn {
  return (control?: AbstractControl): ValidationErrors => {
    ...
    return !galacticCoordinatesRegex.test(control.value)
      ? {
  ...
  if (control instanceof FormGroup || control instanceof FormArray) {
    _.forEach(control.controls, (child: AbstractControl, controlName: string) => {
      const newParentLabel = parentLabel && parentLabel.length > 0
        ...
        : `[${ controlName }]`;
      errors = errors.concat(getAllFormErrors(child, controlName, newParentLabel));
    });
  }
